...
std::vector<std::vector<char>> getAllPerms(PMTree& tree_) {
  std::vector<std::vector<char>> result;
  std::vector<char> path;
  collect(tree_.getRoot(), path, result);
  return result;
}

std::vector<char> getPerm1(PMTree& tree_, int num) {
  auto all = getAllPerms(tree_);
  if (num < 1 || num > all.size()) return {};
  return all[num - 1];
}

std::vector<char> searchPerm(Node* node, int& count) {
  if (!node) return {};
  ...
  for (auto* child : node->children) {
    auto sub = searchPerm(child, count);
    if (!sub.empty()) {
...
std::vector<char> getPerm2(PMTree& tree_, int num) {
  if (num < 1 || num > tree_.getSize()) return {};

  int counter = num;
  return searchPerm(tree_.getRoot(), counter);
}
